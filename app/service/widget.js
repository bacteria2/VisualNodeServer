...
class WidgetService extends Service {
   ...
   async copyWidget(srcWidgetId,newName){
    ...
    async insert(option) {
        ...
        let widget = null
        if(prototype){
            widget = {rawOption:{},prototypeId:ObjectId(prototypeId),projectId,labels,name,type,
        ...
        if(widget.name){
            // 判断同一个项目是否存在同名实例
            const findOne =  await app.mongo.db.collection('widgets').findOne({name: widget.name,projectId:widget.projectId});
   ...
   /* async addWidget(Widget){
    ...
    async saveWidget(widgetId,widget){
        ...
        delete widget._id
        return await this.app.mongo.db.collection('widgets').updateOne({_id: ObjectId(widgetId)},{$set:{
            ...widget
        }});
    }
}
